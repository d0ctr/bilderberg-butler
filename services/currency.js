...
const fetchCurrenciesList = (app) => {
    ...
    (async () => {
        ...
        try {
            ...
            logger.error(`Error while fetching crypto currenies list`, { error: err.stack || err });
        }
        try {
            //get fiat
            ...
            logger.error(`Error while fetching fiat currenies list`, { error: err.stack || err });
        }
    
        if (Object.keys(currencies_list).length > 0) {
            setHealth('currency', 'ready');
            logger.info('Retrieved currencies list');
            app.currencies_list = currencies_list;
            return;
        }
        setHealth('currency', 'fail');
    })();
};
